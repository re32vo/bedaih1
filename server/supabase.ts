...
export async function deleteBeneficiaryRequest(id: string) {
  if (!supabase) return true;
  ...
  if (error) {
    console.error('Error deleting beneficiary request:', error);
    throw new Error('فشل حذف طلب المستفيد: ' + error.message);
  }

  return true;
}

export async function deleteVolunteerRequest(id: string) {
  if (!supabase) return true;
  ...
  if (error) {
    ...
    throw new Error('فشل حذف طلب المتطوع: ' + error.message);
  }
