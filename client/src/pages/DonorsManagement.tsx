...
export default function DonorsManagement() {
  ...
  const [deletingBeneficiary, setDeletingBeneficiary] = useState<BeneficiaryClientRequest | null>(null);
  const [deletingVolunteer, setDeletingVolunteer] = useState<VolunteerClientRequest | null>(null);
  ...
  const handleDeleteBeneficiary = async () => {
    ...
    try {
      const token = sessionStorage.getItem("authToken");
      const res = await fetch(`/api/admin/beneficiaries/${deletingBeneficiary.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.message || "فشل حذف طلب المستفيد");
      }

      toast({
        title: "تم الحذف",
        description: "تم حذف طلب المستفيد بنجاح",
      });
      ...
      toast({
        title: "خطأ",
        description: error instanceof Error ? error.message : "فشل حذف طلب المستفيد",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDeleteVolunteer = async () => {
    ...
    try {
      const token = sessionStorage.getItem("authToken");
      const res = await fetch(`/api/admin/volunteers/${deletingVolunteer.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.message || "فشل حذف طلب المتطوع");
      }

      toast({
        title: "تم الحذف",
        description: "تم حذف طلب المتطوع بنجاح",
      });
      ...
      toast({
        title: "خطأ",
        description: error instanceof Error ? error.message : "فشل حذف طلب المتطوع",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="allow-text-select container mx-auto px-4 py-8">
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors align-top">
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() => setDeletingVolunteer(item)}
                            className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                            title="حذف طلب المتطوع"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors align-top">
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() => setDeletingBeneficiary(item)}
                            className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                            title="حذف طلب المستفيد"
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
      ...
      {/* Delete Beneficiary Request Confirmation */}
      <AlertDialog open={!!deletingBeneficiary} onOpenChange={() => setDeletingBeneficiary(null)}>
        <AlertDialogContent className="bg-slate-900 border-slate-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">هل أنت متأكد؟</AlertDialogTitle>
            <AlertDialogDescription className="text-white/80">
              سيتم حذف طلب المستفيد <strong className="text-white">{deletingBeneficiary?.fullName}</strong> بشكل نهائي. لا يمكن التراجع عن هذا الإجراء.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="text-white border-slate-700 hover:bg-slate-800">إلغاء</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteBeneficiary} className="bg-red-600 hover:bg-red-700 text-white">
              حذف
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      {/* Delete Volunteer Request Confirmation */}
      <AlertDialog open={!!deletingVolunteer} onOpenChange={() => setDeletingVolunteer(null)}>
        <AlertDialogContent className="bg-slate-900 border-slate-700">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">هل أنت متأكد؟</AlertDialogTitle>
            <AlertDialogDescription className="text-white/80">
              سيتم حذف طلب المتطوع <strong className="text-white">{deletingVolunteer?.name}</strong> بشكل نهائي. لا يمكن التراجع عن هذا الإجراء.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="text-white border-slate-700 hover:bg-slate-800">إلغاء</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteVolunteer} className="bg-red-600 hover:bg-red-700 text-white">
              حذف
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
